...
class SendEmailService {
    ...
    private val logger = LogManager.getLogger(SendEmailService::class.java)
    private fun emails(): MimeMessage {
        val properties = Properties()
        val mailSender = mailSenderService.mailSenderInstant
        properties.setProperty("mail.host", mailSender.host)
        properties.setProperty("mail.transport.protocol", mailSender.protocol)
        properties.setProperty("mail.smtp.auth", "true")
        properties.setProperty("mail.default-encoding", mailSender.encoding)
        properties.setProperty("mail.smtp.port", mailSender.port.toString())
        properties.setProperty("mail.smtp.socketFactory.port", mailSender.port.toString())
    ...
    private fun sendEmail(subject: String, nickname: String?, content: String, html: Boolean, vararg toEmail: String?) {
        ...
        mimeMessageHelper.setFrom(InternetAddress(nick + " <" + mailSender.username + ">"))
        logger.info(nick + " <" + mailSender.username + ">")
    ...
    fun sendActivatedEmail(domain: String, user: User) {
        ...
        val url: String = domain + "?" + RSACoder.encryptByPublicKeyToString(String.toJSON(activatedInfo), DatabaseConfig.PublicKey)
        sendHtmlEmail("[SKChatroom]帐号注册激活", "SKChatroom", HtmlFormatBefore + url + HtmlFormatAfter, user.email)
    }

    class ActivatedInfo {
        constructor()
        constructor(username: String, password: String, email: String, expiredTime: Long) : this() {
