...
@Controller
class WebSocketHandler(private val actionMessageService: ActionMessageServiceImpl, private val objectMapper: ObjectMapper) : TextWebSocketHandler() {
    ...
    fun trySendMessage(message: ActionMessage): Boolean {
        actionMessageService.newActionMessage(message) ?: return false
        ...
        return if (isOnline(message.toID)) {
            ...
            return try {
                webSocketMap[message.toID]!!.sendMessage(warpData(String.toJSON(message, objectMapper), "Message"))
                true
            } catch (e: IOException) {
                e.printStackTrace()
                false
